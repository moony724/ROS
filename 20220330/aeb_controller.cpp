...
void odomCallback(const nav_msgs::Odometry& msg)
{
	...
	velocity.twist.twist.linear.x = msg.twist.twist.linear.x;
	velocity.twist.twist.linear.y = msg.twist.twist.linear.y;
	velocity.twist.twist.angular.z = msg.twist.twist.angular.z;
}
...
int main(int argc, char **argv)
 {
  ...
  //ros::Subscriber sub2 = n.subscribe("/RangeSonar1", 1000, UltraSonarCallback2); // sonar`s random value => aeb_controller
    ...
    ros::Subscriber sub_odom_stop = n.subscribe(odom_sub_topic, 10, odomCallback_stop);
	...
	while (ros::ok()) 
	{
		...
		pub_estimated_odom.publish(velocity);
